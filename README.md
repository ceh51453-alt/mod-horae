# Horae - 时光记忆 v1.7.0 | SillyTavern 记忆增强插件
...
**向量记忆引擎 —— 找回被折叠的细节（新）**
专为搭配「自动摘要&隐藏」设计的智能外脑！几十万字长篇压缩后细节全丢？现在当对话触及历史事件时，插件会自动从被隐藏的旧时间线中精准召回相关片段。全程依托 Web Worker 纯本地运算，零 API 消耗，丝滑不卡顿界面（首次运行需下载 30-60MB 模型缓存，提供中文优化的 bge 和多语言 e5 双模型可选）。
